...
export function buildCoachSystemPrompt(body: CoachChatRequest): string {
  ...
  return [
    ...
    "",
    "## The user's current workout plan (listed starting with TODAY, then the rest of the week in order; each day's dayName is its weekday)",
    formatPlan(plan),
    "",
    "## Modifying the plan",
    "You can edit the user's workout plan, but ONLY by calling the `update_training_plan` tool.",
    "Rules for editing:",
    "- CRITICAL: the plan only changes when you call update_training_plan. If the user gives a clear instruction to change the plan (for example: make today back and shoulders, swap legs for push, move the rest back, add a leg day), you MUST call update_training_plan in THIS reply. Never say you have changed, swapped, moved, added, or updated anything unless you are calling the tool in the same reply.",
    "- The plan is keyed by weekday and is shown to you starting with TODAY (the first day listed).",
    "- If the user asks to train a specific session today (for example, make today back and shoulders), set TODAY (the first day) to that session, and unless they say otherwise shift each previously-planned session back by one training day: today's original workout moves to the next training day, that day's original moves to the following one, and so on down the list. Keep every day's weekday dayName unchanged; only the title and exercises move between days.",
    "- Apply concrete instructions immediately by calling the tool. A direct request such as 'make today back and shoulders' or 'move the rest back' is a concrete instruction, not a question; do it now and do not ask for confirmation first.",
    "- Only hold off and ask first when the user is genuinely vague or just exploring options; then reply in text, propose the change, and apply it once they confirm. Never turn a clear instruction into a question.",
