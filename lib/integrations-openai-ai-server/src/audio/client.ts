  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      ffmpeg.on("error", (err: NodeJS.ErrnoException) => {
        // ffmpeg is not installed in some serverless runtimes (e.g. Vercel).
        // Surface a clear, catchable error instead of an opaque ENOENT.
        if (err.code === "ENOENT") {
          reject(
            new Error(
              "ffmpeg is not available in this runtime, so this audio format " +
                "cannot be converted. Record in webm, mp4, mp3, ogg or wav.",
            ),
          );
        } else {
          reject(err);
...
export async function ensureCompatibleFormat(
  ...
  // Whisper / gpt-4o-transcribe accept these containers directly, so we avoid an
  // ffmpeg round-trip (ffmpeg isn't available on some serverless runtimes).
  ...
  // Truly unknown container: best-effort convert (requires ffmpeg).
